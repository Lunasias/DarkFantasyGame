...
import { equipmentModifiers } from "../items/equipment";
...
/**
 * The single authoritative stat-resolution path:
 *   base stats → job modifiers → level modifiers → equipment modifiers → effective stats
 * Used by combat adaptation so the calculation never lives in more than one
 * place. Health is preserved (not reduced) as maxHealth grows, and never exceeds
 * the new maxHealth.
 */
export function effectiveStatsFor(character: Character): CharacterStats {
  ...
  const level = levelStatBonuses(character.level);
  const equip = equipmentModifiers(character);

  const maxHealth = base.maxHealth + (level.maxHealth ?? 0) + (equip.maxHealth ?? 0);
  const attack = base.attack + (level.attack ?? 0) + (equip.attack ?? 0);
  const defense = base.defense + (level.defense ?? 0) + (equip.defense ?? 0);
  return {
    maxHealth,
    health: Math.min(base.health + (level.health ?? 0), maxHealth),
    attack,
    defense,
    speed: base.speed + (level.speed ?? 0) + (equip.speed ?? 0),
  };
}
